...
class RulesTranslator(ast.NodeTransformer):
    ...
    def visit_IfExp(self, C:ast.IfExp):
        if C in self.IDs_map:
            raise Exception("Recursion into an if-expression is not supported yet.")
...
class PreProcessingVisitor(ast.NodeVisitor):
    ...
    class VariablesFinder(ast.NodeVisitor):
        ...
        def addLocal(self, value):
            if not value in self.params:
                self.add(self.locals, value)
        ...
        def visit_Assign(self, node):
            for target in node.targets:
                if isinstance(target, ast.Name):
                    self.addLocal(target.id)
                elif isinstance(target, (ast.Tuple, ast.List)):
                    for element in target.elts:
                        if isinstance(element, ast.Name):
                            self.addLocal(element.id)
            self.generic_visit(node)

        def visit_AnnAssign(self, node):
            if isinstance(node.target, ast.Name):
                self.addLocal(node.target.id)
            self.generic_visit(node)
            
        def visit_With(self, node):
            for item in node.items:
                if item.optional_vars and isinstance(item.optional_vars, ast.Name):
                    self.addLocal(item.optional_vars.id)
            self.generic_visit(node)
            
        def visit_ExceptHandler(self, node):
            if node.name:
                self.addLocal(node.name)
            self.generic_visit(node)
...
if len(sys.argv) <= 1:
    ...
    exit(-1)
infile = sys.argv[1]
#infile = "/home/seguranca/Dropbox/UERJ/Orientacao/repositories/higor/TCO/recursive_functions/casos_excecao_recpython/mdc.py"
f = open(infile)
code = f.read()
f.close()
